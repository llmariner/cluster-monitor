...
build-docker-server:
	docker build --build-arg TARGETARCH=amd64 -t llmariner/cluster-monitor-server:latest -f build/server/Dockerfile .

.PHONY: build-docker-agent
build-docker-agent:
	docker build --build-arg TARGETARCH=amd64 -t llmariner/cluster-monitor-agent:latest -f build/agent/Dockerfile .
