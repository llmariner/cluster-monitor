...
// S is a server.
type S struct {
	v1.UnimplementedClusterMonitorServiceServer
...
func (s *S) Run(ctx context.Context, port int, authConfig config.AuthConfig, usage sender.UsageSetter) error {
	...
	if authConfig.Enable {
		ai, err := auth.NewInterceptor(ctx, auth.Config{
			...
			// TODO(kenji): Revisit. "api.clusters" is currently used by cluster-manager as well.
	...
	v1.RegisterClusterMonitorServiceServer(grpcServer, s)
